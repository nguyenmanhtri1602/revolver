...
(function(w) {
    var H = function() { return w.requestAnimationFrame || w.webkitRequestAnimationFrame || w.mozRequestAnimationFrame || function(a) { w.setTimeout(a, 1E3 / 60) } }(),
    ...
    d.prototype.decodeSocketHeader = function(a) { a[0] === A.charCodeAt(0) && a[1] === A.charCodeAt(1) && a[2] === A.charCodeAt(2) && a[3] === A.charCodeAt(3) && (this.width = 256 * a[4] + a[5], this.height = 256 * a[6] + a[7], this.initBuffers()) };
    d.prototype.receiveSocketMessage = function(a) {
        ...
        b.writePos += c.length;
        for (c = 0;;) { c = b.findNextMPEGStartCode(); if (c === v.NOT_FOUND || b.index >> 3 > b.writePos) { b.index = Math.max(b.writePos - 3, 0) << 3; return } if (c === x) break }
    ...
    d.prototype.stopRecording = function() {
        var a = new Blob(this.recordBuffers, { type: "video/mpeg" });
    ...
    d.prototype.fetchReaderPump = function(a) {
        var c = this;
        a.read().then(function(b) { c.fetchReaderReceive(a, b) })
    };
    ...
    d.prototype.load = function(a) {
        ...
        if (this.progressive && w.fetch && w.ReadableByteStream) {
            ...
            fetch(a, { headers: b }).then(function(a) {
            ...
            f.onreadystatechange = function() { f.readyState === f.DONE && 200 === f.status && c.loadCallback(f.response) };
    ...
    d.prototype.collectIntraFrames = function() {
        var a;
        for (a = 0; this.findStartCode(x) !== v.NOT_FOUND; a++) this.buffer.advance(10), this.buffer.getBits(3) === B && this.intraFrames.push({ frame: a, index: this.buffer.index - 13 });
        this.frameCount = a
    };
    ...
    d.prototype.seekToTime = function(a, c) { this.seekToFrame(a * this.pictureRate | 0, c) };
    d.prototype.play = function() { this.playing || (this.targetTime = this.now(), this.wantsToPlay = this.playing = !0, this.scheduleNextFrame()) };
    d.prototype.pause = function() { this.wantsToPlay = this.playing = !1 };
    ...
    d.prototype.fillArray = function(a, c) { for (var b = 0, f = a.length; b < f; b++) a[b] = c };
    ...
    d.prototype.now = function() { return w.performance ? w.performance.now() : Date.now() };
        ...
        if (this.buffer)
            for (var a = this.now();;) {
                ...
                if (c === E) this.decodeSequenceHeader();
                    ...
                    if (c === x) {
                        if (this.progressive && this.buffer.index >= this.lastFrameIndex) { this.playing = !1; break }
    ...
    d.prototype.scheduleAnimation = function() { H(this.nextFrame.bind(this), this.canvas) };
    ...
    d.prototype.initWebGL = function() {
        var a;
        try { a = this.gl = this.canvas.getContext("webgl") || this.canvas.getContext("experimental-webgl") } catch (b) { return !1 }
        if (!a) return !1;
    ...
    d.prototype.copyBlockToDestination = function(a, c, b, d) { for (var q = 0; 64 > q; q += 8, b += d + 8) c[b + 0] = a[q + 0], c[b + 1] = a[q + 1], c[b + 2] = a[q + 2], c[b + 3] = a[q + 3], c[b + 4] = a[q + 4], c[b + 5] = a[q + 5], c[b + 6] = a[q + 6], c[b + 7] = a[q + 7] };
    d.prototype.addBlockToDestination = function(a, c, b, d) { for (var q = 0; 64 > q; q += 8, b += d + 8) c[b + 0] += a[q + 0], c[b + 1] += a[q + 1], c[b + 2] += a[q + 2], c[b + 3] += a[q + 3], c[b + 4] += a[q + 4], c[b + 5] += a[q + 5], c[b + 6] += a[q + 6], c[b + 7] += a[q + 7] };
    d.prototype.copyValueToDestination = function(a, c, b, d) { for (var q = 0; 64 > q; q += 8, b += d + 8) c[b + 0] = a, c[b + 1] = a, c[b + 2] = a, c[b + 3] = a, c[b + 4] = a, c[b + 5] = a, c[b + 6] = a, c[b + 7] = a };
    d.prototype.addValueToDestination = function(a, c, b, d) { for (var q = 0; 64 > q; q += 8, b += d + 8) c[b + 0] += a, c[b + 1] += a, c[b + 2] += a, c[b + 3] += a, c[b + 4] += a, c[b + 5] += a, c[b + 6] += a, c[b + 7] += a };
    ...
    v.prototype.nextBytesAreStartCode = function() { var a = this.index + 7 >> 3; return a >= this.writePos || 0 === this.bytes[a] && 0 === this.bytes[a + 1] && 1 === this.bytes[a + 2] };
    ...
    v.prototype.advance = function(a) { return this.index += a };
    v.prototype.rewind = function(a) { return this.index -= a }
})(window);
            ...
            if (options.fullscreen) {
                ...
                try {
                    ...
                    if (list.indexOf(prefix.lowercase()) > -1) {
                        if (prefix.lowercase() == "ms") { fsEvent = "MSFullscreenChange"; } else { fsEvent = prefix.lowercase() + fsEvent; }
                    }
                } catch (error) {}
                ...
                if (autoplaybrowser.getOS() == 'ios') {
                    function enterIOSfs() {
                        isFSMode = true;
                        track.openfs();
                        if (typeof(options.fullscreenCallBack) === 'function') { options.fullscreenCallBack("openFS"); }
                    };

                    function exitIOSfs() {
                        isFSMode = false;
                        track.closefs();
                        if (typeof(options.fullscreenCallBack) === 'function') { options.fullscreenCallBack("closeFS"); }
                    };
...
function innityAppsInitCarolCreative() {
    ...
    innityAppsAd = new InnityAppsRevolverLite({
...
function InnityAppsRevolverLite(opts) {
    ...
    // Demo Purposes
    var videoMoving = false,
        reCalculateVideoMove = null;
    ...
    function calcSmallItemPosition_() {
        ...
        let carolRadius = Math.round(carolItemWidth / (2 * Math.sin(itemBaseAngle_ * Math.PI / 360))) - 20;
        ...
        touchSceneAreaEl_.style.marginTop = newMarginTop - 15 + 'px';
    }
    ...
    function rotateToLeft_() {
        ...
        // Demo Purposes
        if (videoMoving) {
            clearTimeout(reCalculateVideoMove);
            reCalculateVideoMove = setTimeout(rotate_, 600);
        }
    ...
    function CarolTouchHandler(toLeftCallback, toRightCallback) {
        ...
        function touchMoveHandler(e) {
            ...
            // Demo Purposes
            let mouseEvtX = typeof e.touches === 'undefined' ? e.clientX : e.touches[0].clientX;
            let xDiff = initX_ - mouseEvtX;
        ...
        function touchStartHandler(e) {
            // Demo Purposes
            initX_ = typeof e.touches === 'undefined' ? e.clientX : e.touches[0].clientX;

            // Demo Purposes
            if (e.target.classList.contains('apps-video-ended-wrapper') || e.target.classList.contains('apps-video-control-wrapper')) {
                videoMoving = true;
            } else {
                videoMoving = false;
            }
